...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  var router = require('cs-router')();

  router.before(component('navigation', '#menu'), excludes).get(paths.teams, component('teams', '#main')).get(paths.teamCreate, component('teamCreate', '#main')).get(paths.teamEdit, component('teamCreate', '#main')).get(paths.races, component('races', '#main')).get(paths.standings, component('standings', '#main')).get(paths.rules, component('rules', '#main'));
});

},{"../middleware/component":299,"../paths":300,"babel-polyfill":1,"cs-router":306}],299:[function(require,module,exports){
"use strict";
...
module.exports = function (name, placeholder) {
  return function () {
    var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(request, response, next, relay) {
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return relay.components.create(name, placeholder);
...
module.exports = Object.assign(paths, { get: get });

},{"./paths":301}],301:[function(require,module,exports){
'use strict';
...
module.exports = function(allHooks, request) {
  const url = parse(request.url);

  return [].concat(
    excludes(allHooks.before),
    routed(allHooks[request.method.toLowerCase()]),
    excludes(allHooks.after)
  );

  function routed(hooks) {
    const matches = hooks.filter(hook => {
      return match(hook.pattern, url);
    });

    // If there is no match found for the current route and method, attach no
    // match hooks and show a warning so it is clear that there wasn't a match
    if (!matches.length && !allHooks.noMatch.length) {
      return [];
    }

    if (!matches.length && allHooks.noMatch.length) {
      return allHooks.noMatch;
    }

    // Add parameters to request object bases on the first match
    request.parameters = parameters(url, matches[0]);

    return matches;
  }

  function parameters(url, hook) {
    let output = {};
    hook.pattern
      .filter(part => !part.static)
      .forEach(part => {
        output[part.value.slice(1)] = url[part.index].value;
      });

    return output;
  }

  // Filter out hooks that have been excluded on the specfic path
  function excludes(hooks) {
    return hooks.filter(hook => {
      return hook.excludes.filter(exclude => {
        return match(exclude, url);
      }).length === 0;
    });
  }

  // Match pattern to url, returns boolean if match
...
},{"./parse":303}],306:[function(require,module,exports){
module.exports = function(server = false) {
  ...
  router.update = (request, response) => {
    // Some request to get become options, readmore here:
    ...
    const current = stack(hooks, request);
    let relays    = {};

    // Set redirect response so redirects can happen on the server side
    redirect = response;
      ...
      return function(relay) {
        if (relay) {
          if (typeof relay === 'object') {
            relays = Object.assign({}, relays, relay);
          } else {
            relays = relay;
          }
        }
        if (hook.callback) {
          hook.callback(request, response, thunkify(current.shift()), relays);
        }
      }
    }

    // Call the first hook
    thunkify(current.shift())();
  };

  function addCallback(method, callback, excludes = []) {

    const errorMessage = 'Exclude urls for a before or after hook needs to be a string or an array of strings';
    ...
    hooks[method].push({
      excludes : excludes.map(parse),
      callback
    });
  }

  function addHook(method, callback, path) {
  ...
  const exposed = {
    before(callback, excludes = []) {
      addCallback('before', callback, excludes);
      return exposed;
    },
    after(callback, excludes = []) {
      addCallback('after', callback, excludes);
      return exposed;
    },
    noMatch(callback) {
      addCallback('noMatch', callback);
      return exposed;
    },
    post(path, callback) {
      addHook('post', callback, path);
      return exposed;
    },
    get(path, callback) {
      addHook('get', callback, path);
      return exposed;
    },
    update(path, callback) {
      addHook('update', callback, path);
      return exposed;
    },
    options(path, callback) {
      addHook('options', callback, path);
      return exposed;
    },
    put(path, callback) {
      addHook('put', callback, path);
      return exposed;
    },
    delete(path, callback) {
      addHook('delete', callback, path);
      return exposed;
    },
    patch(path, callback) {
      addHook('patch', callback, path);
      return exposed;
    },
    head(path, callback) {
      addHook('head', callback, path);
      return exposed;
    },
    redirect(path) {
      if (server) {
        if (path[0] === '/') {
          const domain   = server.address().address === '::' ? 'localhost' : server.address().address;
          const protocol = server.key !== undefined && server.cert !== undefined ? 'https://' : 'http://';
          const port     = server.address().port ? ':' + server.address().port : '';
          redirect.writeHead(302, { location : protocol + domain + port + path });
        } else {
